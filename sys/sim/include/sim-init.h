...
struct SimExported
{
  ...
  int (*sock_socket) (int domain, int type, int protocol, struct SimSocket **socket);
  int (*sock_close) (struct SimSocket *socket);
  ssize_t (*sock_recvmsg) (struct SimSocket *socket, struct msghdr *msg, int flags);
  ssize_t (*sock_sendmsg) (struct SimSocket *socket, const struct msghdr *msg, int flags);
  int (*sock_getsockname) (struct SimSocket *socket, struct sockaddr *name, int *namelen);
  int (*sock_getpeername) (struct SimSocket *socket, struct sockaddr *name, int *namelen);
  int (*sock_bind) (struct SimSocket *socket, const struct sockaddr *name, int namelen);
  int (*sock_connect) (struct SimSocket *socket, const struct sockaddr *name, int namelen, int flags);
  int (*sock_listen) (struct SimSocket *socket, int backlog);
  int (*sock_shutdown) (struct SimSocket *socket, int how);
  int (*sock_accept) (struct SimSocket *socket, struct SimSocket **newSocket, int flags);
  int (*sock_ioctl) (struct SimSocket *socket, int request, char *argp);
  int (*sock_setsockopt) (struct SimSocket *socket, int level, int optname,
			  const void *optval, int optlen);
  int (*sock_getsockopt) (struct SimSocket *socket, int level, int optname,
  ...
  void (*dev_set_address) (struct SimDevice *dev, unsigned char buffer[6]);
  ...
  int (*sys_file_write) (const struct SimSysFile *file, const char *buffer, int size, int offset);
};
...
typedef void (*SimInit) (struct SimExported *, const struct SimImported *, struct SimKernel *kernel);
