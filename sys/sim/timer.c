...
static void run_timer_softirq(void *args)
{
  while (!LIST_EMPTY(&g_expired_events))
    {
      struct callout *c = LIST_FIRST(&g_expired_events);
      void (*fn)(void *);
      void *data;
      fn = c->c_func;
      data = c->c_arg;
...
static void timer_trampoline (void *context)
{
  ...
  LIST_REMOVE(c, c_links.le);
  LIST_INSERT_HEAD(&g_expired_events, c, c_links.le);
  sim_softirq_wakeup ();
    ...
    void (*ftn)(void *), void *arg, int cpu, int flags)
{
  ...
  __u64 delay_ns = 0;
  delay_ns = (__u64) (sbt / tick_sbt) * (1000000000/hz);
  ...
  LIST_INSERT_HEAD(&g_pending_events, c, c_links.le);
